...
struct NoPilha
{
    fichaAtendimento dado; //informação do nó
...
//push
bool empilhaP(Pilha *p, fichaAtendimento dado)
{
...
//pop
fichaAtendimento desempilhaP(Pilha *p)
{
...
//peek
fichaAtendimento espiaP(Pilha* p)
{
    fichaAtendimento dado;
...
void mostraP(Pilha *p)
{
    ...
    if(vaziaP(p) == false)
    {
        ...
        while (no != NULL)
        {
            ...
            cout << setfill(' ') << std::setw(13) << no->dado.nome << " | ";
            cout << setfill(' ') << std::setw(13) << no->dado.idade << " | ";
            cout << setfill(' ') << std::setw(13) << no->dado.tipo << " | ";
            cout << setfill(' ') << std::setw(10) << no->dado.quadroClinico << " |" << endl;
...
bool buscaP(Pilha *p, fichaAtendimento dado)
{
