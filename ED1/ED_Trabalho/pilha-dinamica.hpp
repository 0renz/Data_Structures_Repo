...
struct NoPilha
{
    FichaCliente dado; //informação do nó
...
//push
bool empilhaP(Pilha *p, FichaCliente dado)
{
...
//pop
FichaCliente desempilhaP(Pilha *p)
{
    FichaCliente dado;
...
void mostraP(Pilha *p)
{
    ...
    if(vaziaP(p) == false)
    {
        ...
        while (no != NULL)
        {
            ...
            cout << setfill(' ') << std::setw(10) << no->dado.nome << " |";
            cout << setfill(' ') << std::setw(10) << no->dado.codigo << " |";
            cout << setfill(' ') << std::setw(10) << no->dado.idade << " |";
            cout << setfill(' ') << std::setw(10) << no->dado.saldoEmConta << " |" << endl;
...
bool buscaP(Pilha *p, FichaCliente dado)
{
